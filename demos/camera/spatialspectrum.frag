...
void main() {
	...
	if (rawBeamspace == 1 && (sourceCoordBeamspace.x < 0 || sourceCoordBeamspace.x > 1 || sourceCoordBeamspace.y < 0 || sourceCoordBeamspace.y > 1)) {
		s = vec4(0.0);
	}
	...
	// dot/trace overlay, only visible in polarization "show" mode
	if (polarizationVisible == 1) {
		...
		totalContribution = max(pointContribution, traceContribution * 0.6 + glowContribution);	
	}
